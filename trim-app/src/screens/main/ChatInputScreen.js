...
const UNITS = ['serving', 'bowl', 'plate', 'cup', 'slice', 'piece', 'g', 'ml'];

const SMART_QTY = { g: '100', ml: '250', serving: '1', bowl: '1', plate: '1', cup: '1', slice: '1', piece: '1' };

const emptyItem = () => ({ food: '', quantity: '1', unit: 'serving' });
...
const enrich = (item) => ({
  ...
  displayCalories: item.calories,
  displayProtein:  item.protein,
  displayCarbs:    item.carbs,
  displayFat:      item.fat,
});

const ChatInputScreen = ({ navigation }) => {
  const { user } = useAuth();
  ...
  const hasInput      = formItems.some((i) => i.food.trim()) || freeText.trim().length > 0;
  ...
  const updateQty = (index, dir) => {
    const item = formItems[index];
    const isMetric = ['g', 'ml'].includes(item.unit);
    const step = isMetric ? 50 : 0.5;
    const min  = isMetric ? 50 : 0.5;
    const current = parseFloat(item.quantity) || 1;
    const next = Math.max(min, current + dir * step);
    updateItem(index, 'quantity', Number(next.toFixed(1)).toString());
  };
  ...
  const getHalfServingText = () => parsedItems.length === 1 ? '0.5x serving' : 'half portions';

  const updateDisplayCalories = (index, newVal) => {
    const updated = parsedItems.map((item, i) =>
      i === index ? { ...item, displayCalories: newVal } : item
    );
    setParsedItems(updated);
  ...
  const handleParse = async () => {
    const formParts = formItems
      ...
      .map((it) => `${it.quantity} ${it.unit} of ${it.food}`);
    ...
    try {
      ...
      setParsedItems((res.data.items || []).map(enrich));
    ...
    setParsedItems((prev) =>
      prev.map((item, i) => {
        ...
        return {
          ...item,
          multiplier,
          displayCalories: Math.round(item.calories * multiplier),
          displayProtein:  Math.round(item.protein  * multiplier),
          displayCarbs:    Math.round(item.carbs    * multiplier),
          displayFat:      Math.round(item.fat      * multiplier),
        };
      })
    );
  ...
  const handleLogAll = async () => {
    ...
    const body = {
      ...
      items: parsedItems.map((item) => ({
        name:            item.name,
        calories:        item.displayCalories,
        protein:         item.displayProtein,
        carbs:           item.displayCarbs,
        fat:             item.displayFat,
        servingSize:     item.servingSize || '',
        servingQuantity: parseFloat(item.multiplier) || 1,
        source:          'ai_parsed',
      })),
    };
    console.log('Logging items:', JSON.stringify(body, null, 2));
    try {
  ...
  const renderTopContent = () => {
      ...
      return (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 32 }}>
          ...
          <Text style={{ color: '#666', fontSize: 15, textAlign: 'center', marginBottom: 32 }}>
            Added to your {mealTypeLabel} log
          </Text>
          ...
          <View style={{
            ...
            <Text style={{ color: '#555', fontSize: 12, textAlign: 'center', marginTop: 2 }}>
              {parsedItems.length} item{parsedItems.length > 1 ? 's' : ''} logged
            </Text>
          </View>
    ...
    if (uiState === 'results') {
      // ── Insight block computation ────────────────────────────────────────────
      const afterMeal = remainingCalories != null ? remainingCalories - displayTotal : null;
      ...
      if (afterMeal == null) {
        ...
        if (afterMeal > 0) {
          insightIcon = '💪'; insightColor = '#64B5F6';
          insightText = `Still need ${afterMeal} cal today`;
          insightSub  = 'Keep eating to hit your goal';
        } else {
          insightIcon = '✅'; insightColor = '#2ECC71';
          insightText = "You've hit today's calorie goal!";
          insightSub  = 'Great work';
        }
      } else {
        if (afterMeal >= 0) {
          insightIcon = '✅'; insightColor = '#2ECC71';
          insightText = `${afterMeal} cal remaining after this meal`;
          insightSub  = 'Fits your daily goal';
        } else if (afterMeal >= -200) {
          insightIcon = '⚠️'; insightColor = '#FFA726';
          insightText = `${Math.abs(afterMeal)} cal over your budget`;
          insightSub  = `Try ${getHalfServingText()} to stay on track`;
        } else {
          insightIcon = '🔴'; insightColor = '#EF5350';
          insightText = `${Math.abs(afterMeal)} cal over your budget`;
          insightSub  = 'Consider a smaller portion';
        }
      }
      ...
      return (
        <>
          {parsedItems.map((item, i) => (
            <View key={i} style={[styles.itemCard, { position: 'relative' }]}>
              ...
              <Text style={styles.itemMacros}>
                {item.displayCalories} cal  ·  P {item.displayProtein}g  ·  C {item.displayCarbs}g  ·  F {item.displayFat}g
              </Text>
              ...
              <View style={styles.servingRow}>
                ...
                {MULTIPLIERS.map((m) => (
                  <TouchableOpacity
                    key={m}
                    style={[styles.servingPill, item.multiplier === m && styles.servingPillOn]}
                    onPress={() => setItemMultiplier(i, m)}
                  >
                    <Text style={[styles.servingPillText, item.multiplier === m && styles.servingPillTextOn]}>
                      {m}x
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              {editingIndex === i && (
                <View style={{ backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: 12, padding: 12, marginTop: 10 }}>
                  <Text style={{ color: '#666', fontSize: 11, marginBottom: 4 }}>CALORIES</Text>
          ...
          <View style={styles.totalBar}>
            <Text style={styles.totalText}>Total: {displayTotal} cal</Text>
          </View>
      ...
      <>
        <View style={styles.formHeader}>
          ...
          <Text style={{ color: '#666', fontSize: 14, marginTop: 4 }}>
            Type in any language — English, Vietnamese, or more
          </Text>
        </View>
        ...
        {formItems.map((item, i) => (
          <View key={i} style={styles.formItem}>
            {/* Row 1: food input + stepper */}
            <View style={styles.formRow}>
              <TextInput
                style={[styles.baseInput, { flex: 1 }]}
                value={item.food}
                onChangeText={(v) => updateItem(i, 'food', v)}
                placeholder="Dish name & details (e.g. pho bo less soup)"
                placeholderTextColor="#444"
                color="#FFFFFF"
                fontSize={14}
              />
              <View style={styles.stepper}>
                <TouchableOpacity style={styles.stepperBtn} onPress={() => updateQty(i, -1)}>
                ...
                <TextInput
                  ...
                  onChangeText={(val) => {
                    if (/^\d*\.?\d*$/.test(val)) updateItem(i, 'quantity', val);
                  }}
                  onBlur={() => {
                    const val = parseFloat(item.quantity);
                    if (!val || val < 0.5) updateItem(i, 'quantity', '0.5');
                  }}
              ...
              {UNITS.map((unit) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    updateItem(i, 'quantity', SMART_QTY[unit] || '1');
                  }}
        ...
        <TextInput
          ...
          placeholder="e.g. 2 bowls of pho with extra beef, 1 iced coffee less sugar..."
          placeholderTextColor="#444"
          color="#FFFFFF"
          fontSize={14}
          multiline
        ...
        <>
          ...
          >
            {logging
              ...
              : <Text style={styles.primaryBtnText}>Log to {mealTypeLabel}</Text>
            }
          </TouchableOpacity>
    ...
    return (
      <>
        ...
        <Text style={{ color: '#444', fontSize: 11, textAlign: 'center', marginTop: 8 }}>
          ⚡ AI estimates · adjust servings after parsing
        </Text>
      </>
    );
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, alignItems: 'center' }}>
          <Text style={styles.headerTitle}>AI Food Log</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
...
const styles = StyleSheet.create({
  ...
  successTitle:  { color: '#FFFFFF', fontSize: 22, fontWeight: '700', marginTop: 12 },
  successSub:    { color: '#666', fontSize: 14, marginTop: 6 },
  ...
  servingRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8 },
  servingLabel:        { color: '#555', fontSize: 12, marginRight: 2 },
  servingPill:         { backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 4 },
  servingPillOn:       { backgroundColor: '#2ECC71' },
  servingPillText:     { color: '#888', fontSize: 12 },
  servingPillTextOn:   { color: '#0F0F0F', fontWeight: '700' },
